...
from link_google import load_pdfs_from_google_drive
import pickle
...
def process_documents(source, folder_path=None, folder_id=None):
    ...
    if os.path.exists(vector_store_path):
        ...
        if source == "local":
            ...
            documents = load_pdfs_from_google_drive(folder_id)
...
source = sidebar.radio("Select source:", ("Local Directory", "Google Drive"))
    ...
    if folder_path:
        if "vectorstore" not in st.session_state:
            with st.spinner("Processing documents..."):
                st.session_state.vectorstore = process_documents("local", folder_path=folder_path)
        sidebar.markdown(f"**Resource Address:** \n Local Folder Path:{folder_path}")
elif source == "Google Drive":
    folder_id = sidebar.text_input("Enter the folder ID on Google Drive:")
    if folder_id:
        if "vectorstore" not in st.session_state:
            with st.spinner("Processing documents..."):
                st.session_state.vectorstore = process_documents("google_drive", folder_id=folder_id)
...
if "vectorstore" in st.session_state:
    if sidebar.button("Reset"):
        ...
        if os.path.exists("vector_store_api"):
            os.remove("vector_store_api")
        st.success("Vector store reset.")


if folder_path or folder_id:
    if "vectorstore" not in st.session_state:
        with st.spinner("Processing documents..."):
            st.session_state.vectorstore = process_documents(source, folder_path=folder_path, folder_id=folder_id)
